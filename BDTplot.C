...
// MC FILEs
const char * files[] = {
    "/user/u/u25pedrochan/TMVA_Bu/TMVA/MC_Bu_BDTScore.root",     //MC BDT Score ppRef                         
    //"/lstore/cms/henrique/Bmeson/MC_DATA/MC_ppRef_Bmeson/Bd_phat5_Bfinder.root"         //ppRef
   // "/lstore/cms/henrique/Bmeson/MC_DATA/MC_ppRef_Bmeson/Bs_phat5_Bfinder.root"       //ppRef
...
const char* files_data={
    "/user/u/u25pedrochan/TMVA_Bu/TMVA/Data_LekaiCut_Bu_BDTScore.root"    //RightsideBand B+ BDT Score ppRef
    //"/lstore/cms/henrique/Bmeson/MC_DATA/DATA_ppref_Bmeson/DATA_ppref_Bmeson.root" // Dados completos
...
//VARIABLES
const char * variables[] = {/*"BDT_score",*/  "B_mass"};
const double ranges[][2] = { /*{-1 , 1},*/    {5, 6} };
...
for (int ifile = 0; ifile < sizeof(files)/sizeof(files[0]); ++ifile) {
    ...
    if (path_to_file.Contains("Bs")){
        file->GetObject("tree", treeMix);
        file_data->GetObject("tree", treedata); 
    ...
    for (int i = 0; i < nVars; ++i) {
        ...
        TH1F *hist_MCBDT = new TH1F("hist_MCBDT"    , Form("; %s; Entries / %.3f ", Xlabel.Data(), bin_length_MEV) , hist_Nbin, hist_Xlow ,hist_Xhigh);
        TH1F *hist_BKGBDT = new TH1F("hist_BKGBDT"      , Form("; %s; Entries / %.3f ", Xlabel.Data(), bin_length_MEV) , hist_Nbin, hist_Xlow ,hist_Xhigh);
        ...
        if (path_to_file.Contains("Bd")){ 
	      ...
	      treedata->Draw(Form("%s >> hist_BKG"     , var.Data()), Form("Bnorm_svpvDistance_2D>3.9916 && Bchi2cl>0.05 && %s && %s", cut.Data(), sepcCASES.Data()));       // BKG -- (notice the *!* in the first %s)
           ...
           //treeMix->Draw(Form("%s >> hist_SIG", var.Data()), Form("Bnorm_svpvDistance>2 && Bchi2cl>0.05 && %s && %s && %s", isMCsignal.Data(), cut.Data(), sepcCASES.Data())); 
           treedata->Draw(Form("%s >> hist_BKGBDT", var.Data()), Form("BDT_score>0.01944"));  // BKG -- (notice the *!* in the first %s)
        } else if(path_to_file.Contains("Rho")) {
        ...
        hist_BKGBDT->SetLineColor(kBlue);
        hist_BKGBDT->SetFillColor(kBlue);     
        hist_BKGBDT->SetFillStyle(3358);
        ...
        hist_BKGBDT->SetName("DATA");  // <--- Also affects the stat box
...
hist_SIG_BOTH->SetStats(kFALSE);
        ...
        TPaveStats *st_bkg = (TPaveStats*)hist_BKGBDT->GetListOfFunctions()->FindObject("stats");
        if (st_bkg) {
        ...
        if(0){
            ...
            double Nbkg = hist_BKGBDT->GetEntries();
        ...
        legend->AddEntry(hist_BKGBDT, "DATA", "l");
        // legend->Draw();    


        // Save the canvas as an image
        TString particleNAME = "Bu";
        TString systemNAME = "DATA_BDT_ppRef_";
        if (path_to_file.Contains("Bs")){
            particleNAME = "Bs";
        } else if (path_to_file.Contains("Bd")){
            particleNAME = "Bd";
